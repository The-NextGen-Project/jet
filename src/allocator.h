...
namespace nextgen::mem { using namespace nextgen::core;
    ...
    namespace arena {
...
#ifdef NG_OS_WINDOWS
        static_cast<uint8_t*>(VirtualAlloc(nullptr,OVER_COMMIT_MEMORY_SIZE,MEM_RESERVE,PAGE_READWRITE));
#endif
#ifdef NG_OS_LINUX
      ...
      NG_INLINE uint8_t *give_and_commit(decltype(UINTPTR_MAX) memory) {
        ...
        // same buffer.
        return static_cast<uint8_t*>(VirtualAlloc(mem, memory, MEM_COMMIT, PAGE_READWRITE));
#else
        return mem;
#endif
      }
...
#ifdef NG_OS_WINDOWS
        (void) VirtualAlloc(arena, memory, MEM_COMMIT, PAGE_READWRITE);
#endif
      }
    }
...
#define COND(x) [&](){ return x; }
#define ALLOC_IF(type, cond, ...) mem::alloc_if<type>(COND(cond), [&](auto _) { __VA_ARGS__ })
    template<typename Alloc, typename Condition, typename Action>
    static constexpr auto alloc_if(Condition condition, Action action) {
      const auto begin = (Alloc*) mem::GLOBAL_OBJECT_ALLOC.current();
      const auto _     = (Alloc*) nullptr;
      while (condition()) {
        action(_);
      }
      return std::vector<Alloc>(begin, (Alloc*) new char('a'));
    }
