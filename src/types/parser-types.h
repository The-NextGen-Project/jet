...
namespace nextgen { namespace jet {
    ...
    public: // Impl
      [[nodiscard]] NG_AINLINE bool has_typename() const {
        return this->ty_name.is_some();
      }

      [[nodiscard]] NG_AINLINE bool has_modifier() const {
        return this->modifier.is_some();
      }

      [[nodiscard]] NG_AINLINE bool has_type() const {
        return this->type;
      }
      ...
      [[nodiscard]] Syntax_Typename get_typename() const {
        if (!has_typename()) {
    ...
    struct Syntax_Elif : public Syntax_Node {
      ...
      explicit Syntax_Elif(Syntax_Block block, Syntax_Elif *elif) : body(block), next(elif) {}
    ...
    public: // Constructors
      Syntax_ForList(const Token *ln, const Token *lv, Syntax_Block body)
        ...
        this->kind = Syntax_Kind::For_List;
      }
    };
    ...
    public: // Constructors
      Syntax_ForRange(const Token *list_var, const Range_Function range, Syntax_Block body)
        ...
        this->kind = Syntax_Kind::For_Range;
      }
    };

    struct Break : public Syntax_Node { Break() { this->kind = Syntax_Kind::Break; }};
    struct Continue : public Syntax_Node { Continue() { this->kind = Syntax_Kind::Continue; }};
    struct Defer : public Syntax_Node {
      const Syntax_Node *expression;
    public:
      Defer(const Syntax_Node *expr) : expression(expr) { this->kind = Syntax_Kind::Defer; }
    };
