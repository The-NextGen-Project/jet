...
//    Console::Log(value), next<Mode>((sizeof value) - 1); \
//else                                 \
//  (L.tokens.end = TOKEN_SHORT(value, kind), next<Mode>((sizeof value) - 1)) \
//\
//
...
//  Range<Token *>tokens{};
//public:
//  auto peek(size_t chars) { return *(buffer + chars); }
//  auto curr() { return *buffer; }
//
//  template<Lex_Mode Mode>
//  auto next(size_t n) {
//    if (buf_pos >= buf_size) {
//      if (Mode is TokenMode) {
...
//    if (::strncmp(Reserved[i].id, ident, len) is 0)
...
//    lex_float<Mode>(L.buffer - token_start, lexed_int);
//    return;
//  }
//
...
//        case '.': lex_float<Mode>(L, 2); return;
...
//  const char *token_start = L.buffer;
//  auto token_start_col = L.column;
//  do {} while(MatchTokenKind[L.next<Mode>(1)] is Token_Kind::Identifier);
//  const char *token_end   = L.buffer;
//
//  auto literal_token_representation = std::string_view(token_start, token_end-token_start);
...
//          case 'x': {
//            auto value = IntegerBits[L.next<Mode>(1)];
//            if (value >= 16) {
//              L.diagnostics.build(Lex_Error{
...
//              });
//            }
//            auto value2 = IntegerBits[L.next<Mode>(1)];
//            if (value2 >= 16) {
//              L.diagnostics.build(Lex_Error{
//                Lex_Error_Kind::Hex_Escape_Out_Of_Range,
//                {L.line, L.column},
...
//      Console::Log(next_ch);
//    ++L.line, L.column = 1, L.next<Mode>(1);
//  }
//}
//
//template<Lex_Mode Mode>
...
//          Console::Log(L.next<Mode>(1), Colors::RESET);
//        }
//        else {
//          auto ch = L.next<Mode>(1);
//          L.tokens.end = new Token {
...
//          else if (n is '/') {
//            if constexpr (Mode is PrintingMode) Console::Log("/");
//            while (MatchTokenKind[L.next<Mode>(1)] != NewLine) {
//              if constexpr (Mode is PrintingMode) {
//                Console::Log(L.curr());
//              }
//            }
...
//  template<> std::vector<Token> lex<TokenMode>(const char *, const char *, size_t);
//  template<> std::vector<Token> lex<PrintingMode>(const char *, const char *, size_t);
//}